...
* **Dismissal:** Modals must be closable by: clicking the × button, pressing Escape, or clicking the backdrop. Never trap a user in a modal.
* **Stacked Modals:** Modals must not open other modals (no stacked modals). If additional confirmation is needed, use inline messaging or inline warnings within the existing modal.
* **Dirty Form Dismissal Guard:** If the Dialog contains a form, you MUST check if the form is dirty (`isDirty`). If dirty, show a confirmation dialog (`DiscardChangesConfirmationDialog`) before dismissing.
* **Save State:** Modals that contain forms must save draft state if closed accidentally. Never lose a user's input because they accidentally pressed Escape.
