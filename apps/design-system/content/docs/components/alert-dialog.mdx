...
Assembly’s Alert Dialog extends the Radix/Shadcn version with async action handling. When an
...
- **Use for dirty-form discard confirmation:** A short discard-confirmation step after a dirty form dismissal attempt (backdrop, Escape, or `Cancel`) is a valid Alert Dialog pattern. Prefer `DiscardChangesConfirmationDialog` or a similar dedicated confirmation helper for this flow.
...
* **Hierarchy:** Destructive actions must never be the primary action on any screen. They must be visually de-emphasized (e.g. secondary outline button, or red text link) until inside the confirmation dialog.
